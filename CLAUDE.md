...
- **Run Playwright tests before merging any PR.** `CHROMIUM=/usr/bin/chromium npm test` from the repo root. All suites must pass (or pre-existing failures must be confirmed unchanged) before the branch merges. Fix any new test failures introduced by the PR before merging. Do not install Playwright in CI — run it locally in the session.
